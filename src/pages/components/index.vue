<template>
  <view v-if="loading" class="uno-p-15px">
    <u-skeleton rows="3" title loading></u-skeleton>
  </view>

  <template v-else>
    <u-cell-group title="基础组件">
      <u-cell icon="grid" title="Color 色彩" is-link url="/pagesA/base/color" />
      <u-cell
        icon="grid-fill"
        title="Icon 图标"
        is-link
        url="/pagesA/base/icon"
      />
      <u-cell
        icon="photo-fill"
        title="Image 图片"
        is-link
        url="/pagesA/base/image"
      />
      <u-cell
        icon="plus-square-fill"
        title="Button 按钮"
        is-link
        url="/pagesA/base/button"
      />
      <u-cell
        icon="file-text"
        title="Text 文本"
        is-link
        url="/pagesA/base/text"
      />
      <u-cell
        icon="grid"
        title="Layout 布局"
        is-link
        url="/pagesA/base/layout"
      />
      <u-cell icon="list" title="Cell 单元格" is-link url="/pagesA/base/cell" />
      <u-cell
        icon="bookmark"
        title="Badge 微标数"
        is-link
        url="/pagesA/base/badge"
      />
      <u-cell
        icon="bookmark-fill"
        title="Tag 标签"
        is-link
        url="/pagesA/base/tag"
      />
      <u-cell
        icon="reload"
        title="Loading 加载动画"
        is-link
        url="/pagesA/base/loading-icon"
      />
      <u-cell
        icon="reload"
        title="Loading page 加载页"
        is-link
        url="/pagesA/base/loading-page"
      />
    </u-cell-group>

    <u-cell-group title="表单组件">
      <u-cell icon="order" title="Form 表单" is-link url="/pagesA/form/form" />
      <u-cell
        icon="calendar"
        title="Calendar 日历"
        is-link
        url="/pagesA/form/calendar"
      />
      <u-cell
        icon="chat"
        title="Keyboard 键盘"
        is-link
        url="/pagesA/form/keyboard"
      />
      <u-cell
        icon="more-circle"
        title="Picker 选择器"
        is-link
        url="/pagesA/form/picker"
      />
      <u-cell
        icon="clock"
        title="DatetimePicker 时间选择器"
        url="/pagesA/form/datetime-picker"
        is-link
      />
      <u-cell icon="star" title="Rate 评分" is-link url="/pagesA/form/rate" />
      <u-cell
        icon="search"
        title="Search 搜索"
        is-link
        url="/pagesA/form/search"
      />
      <u-cell
        icon="plus"
        title="NumberBox 步进器"
        is-link
        url="/pagesA/form/number-box"
      />
      <u-cell
        icon="arrow-upward"
        title="Upload 上传"
        is-link
        url="/pagesA/form/upload"
      />
      <u-cell
        icon="email"
        title="Code 验证码倒计时"
        is-link
        url="/pagesA/form/code"
      />
      <u-cell
        icon="edit-pen"
        title="Input 输入框"
        is-link
        url="/pagesA/form/input"
      />
      <u-cell
        icon="edit-pen"
        title="Textarea 文本域"
        is-link
        url="/pagesA/form/textarea"
      />
      <u-cell
        icon="checkmark-circle"
        title="Checkbox 复选框"
        is-link
        url="/pagesA/form/checkbox"
      />
      <u-cell
        icon="checkbox-mark"
        title="Radio 单选框"
        is-link
        url="/pagesA/form/radio"
      />
      <u-cell
        icon="checkmark-circle-fill"
        title="Switch 开关选择器"
        is-link
        url="/pagesA/form/switch"
      />
      <u-cell
        icon="integral"
        title="Slider 滑动选择器"
        is-link
        url="/pagesA/form/slider"
      />
      <u-cell
        icon="photo-fill"
        title="Album 相册"
        is-link
        url="/pagesA/form/album"
      />
    </u-cell-group>

    <u-cell-group title="数据组件">
      <u-cell
        icon="list-dot"
        title="List 列表"
        is-link
        url="/pagesA/data/list"
      />
      <u-cell
        icon="car"
        title="Progress 进度条"
        is-link
        url="/pagesA/data/progress"
      />
      <u-cell
        icon="clock-fill"
        title="CountDown 倒计时"
        is-link
        url="/pagesA/data/count-down"
      />
      <u-cell
        icon="hourglass"
        title="CountTo 数字滚动"
        is-link
        url="/pagesA/data/count-to"
      />
    </u-cell-group>

    <u-cell-group title="反馈组件">
      <u-cell
        icon="order"
        title="Tooltip 长按提示"
        is-link
        url="/pagesA/feedback/tooltip"
      />
      <u-cell
        icon="list"
        title="ActionSheet 上拉菜单"
        is-link
        url="/pagesA/feedback/action-sheet"
      />
      <u-cell
        icon="warning"
        title="Alert 警告提示"
        is-link
        url="/pagesA/feedback/alert"
      />
      <u-cell
        icon="chat"
        title="Toast 消息提示"
        is-link
        url="/pagesA/feedback/toast"
      />
      <u-cell
        icon="chat-fill"
        title="Noticebar 滚动通知"
        is-link
        url="/pagesA/feedback/notice-bar"
      />
      <u-cell
        icon="volume"
        title="Notify 消息提示"
        is-link
        url="/pagesA/feedback/notify"
      />
      <u-cell
        icon="list-dot"
        title="SwipeAction 滑动单元格"
        is-link
        url="/pagesA/feedback/swipe-action"
      />
      <u-cell
        icon="bookmark"
        title="Collapse 折叠面板"
        is-link
        url="/pagesA/feedback/collapse"
      />
      <u-cell
        icon="share-square"
        title="Popup 弹出层"
        is-link
        url="/pagesA/feedback/popup"
      />
      <u-cell
        icon="more-circle-fill"
        title="Modal 模态框"
        is-link
        url="/pagesA/feedback/modal"
      />
    </u-cell-group>

    <u-cell-group title="布局组件">
      <u-cell
        icon="coupon"
        title="ScrollList 横向滚动列表"
        is-link
        url="/pagesA/layout/scroll-list"
      />
      <u-cell
        icon="minus"
        title="Line 线条"
        is-link
        url="/pagesA/layout/line"
      />
      <u-cell
        icon="moments-circel-fill"
        title="Overlay 遮罩层"
        is-link
        url="/pagesA/layout/overlay"
      />
      <u-cell
        icon="wifi-off"
        title="NoNetwork 无网络提示"
        is-link
        url="/pagesA/layout/no-network"
      />
      <u-cell
        icon="grid"
        title="Grid 宫格布局"
        is-link
        url="/pagesA/layout/grid"
      />
      <u-cell
        icon="gift-fill"
        title="Swiper 轮播图"
        is-link
        url="/pagesA/layout/swiper"
      />
      <u-cell
        icon="more-dot-fill"
        title="Skeleton 骨架屏"
        is-link
        url="/pagesA/layout/skeleton"
      />
      <u-cell
        icon="info"
        title="Sticky 吸顶"
        is-link
        url="/pagesA/layout/sticky"
      />
      <u-cell
        icon="minus"
        title="Divider 分割线"
        is-link
        url="/pagesA/layout/divider"
      />
    </u-cell-group>

    <u-cell-group title="导航组件">
      <u-cell
        icon="bookmark"
        title="Tabbar 底部导航栏"
        is-link
        url="/pagesA/nav/tabbar"
      />
      <u-cell
        icon="arrow-up"
        title="BackTop 返回顶部"
        is-link
        url="/pagesA/nav/back-top"
      />
      <u-cell
        icon="home-fill"
        title="Nabbar 导航栏"
        is-link
        url="/pagesA/nav/navbar"
      />
      <u-cell
        icon="tags-fill"
        title="Tabs 标签"
        is-link
        url="/pagesA/nav/tabs"
      />
      <u-cell
        icon="tags"
        title="Subsection 分段器"
        is-link
        url="/pagesA/nav/subsection"
      />
      <u-cell
        icon="list-dot"
        title="IndexList 索引列表"
        is-link
        url="/pagesA/nav/index-list"
      />
      <u-cell
        icon="skip-forward-right"
        title="Steps 步骤条"
        is-link
        url="/pagesA/nav/steps"
      />
      <u-cell
        icon="coupon"
        title="Empty 空内容"
        is-link
        url="/pagesA/nav/empty"
      />
    </u-cell-group>

    <u-cell-group title="其他组件">
      <u-cell
        icon="file-text-fill"
        title="Parse 富文本解析器"
        is-link
        url="/pagesA/other/parse"
      />
      <u-cell
        icon="edit-pen"
        title="CodeInput 验证码输入"
        is-link
        url="/pagesA/other/code-input"
      />
      <u-cell
        icon="more-circle"
        title="Loadmore 加载更多"
        is-link
        url="/pagesA/other/loadmore"
      />
      <u-cell
        icon="map"
        title="ReadMore 展开阅读更多"
        is-link
        url="/pagesA/other/read-more"
      />
      <u-cell icon="scan" title="Gap 间隔槽" is-link url="/pagesA/other/gap" />
      <u-cell
        icon="server-man"
        title="Avatar 头像"
        is-link
        url="/pagesA/other/avatar"
      />
      <u-cell
        icon="attach"
        title="Link 超链接"
        is-link
        url="/pagesA/other/link"
      />
      <u-cell
        icon="twitter"
        title="transition 动画"
        is-link
        url="/pagesA/other/transition"
      />
    </u-cell-group>
  </template>
</template>

<script lang="ts" setup>
  import { ref } from 'vue';

  const loading = ref(true);

  setTimeout(() => {
    loading.value = false;
  }, 100);
</script>

<style lang="scss" scoped></style>
