<template>
  <view>
    <u-text text="基本案例" type="info" margin="10px 0 10px 10px" />
    <view>
      <u-grid :border="false" @click="click" align="center">
        <u-grid-item v-for="(baseListItem, baseListIndex) in baseList" @click="click('test')" :key="baseListIndex">
    ...
    <u-text text="显示边框" type="info" margin="10px 0 10px 10px" />
    <view>
      <u-grid :border="true">
        <u-grid-item
    ...
    <u-text text="绑定点击事件&自定义列数" type="info" margin="10px 0 10px 10px" />
    <view>
    ...
    <u-text text="可滑动" type="info" margin="10px 0 10px 10px" />
    <view>
...
<script setup lang="ts">
  ...
  const click = (name: string | number | undefined) => {
    console.log(name);
    Toast(`点击了第${name}个`);
  };
</script>
