<template>
  <view class="_u_p-15px">
    <u-text type="info" margin="0 0 15px 0" text="基础功能"></u-text>
    <u-text text="我用十年青春,赴你最后之约" @click="test"></u-text>

    <u-text type="info" margin="15px 0" text="设置主题"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text text="主色" type="primary"></u-text>
      </view>
      <view class="u-page__text-item">
        <u-text type="error" text="错误"></u-text>
      </view>
      <view class="u-page__text-item">
        <u-text type="success" text="成功"></u-text>
      </view>
      <view class="u-page__text-item">
        <u-text type="warning" text="警告"></u-text>
      </view>
      <view class="u-page__text-item">
        <u-text type="info" text="信息"></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="拨打电话"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text mode="phone" text="15019479320"></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="日期格式化"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text mode="date" text="1612959739"></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="姓名脱敏"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text mode="name" text="张三三" format="encrypt"></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="超链接"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text
          mode="link"
          text="Go to uView docs"
          href="https://www.uviewui.com"
        ></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="显示金额"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item">
        <u-text mode="price" text="728732.32"></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="前后图标"></u-text>
    <view class="u-demo-block__content">
      <view class="u-page__text-item" style="margin-right: 50px">
        <u-text
          prefixIcon="baidu"
          iconStyle="font-size: 19px"
          text="百度一下"
        ></u-text>
      </view>
      <view class="u-page__text-item">
        <u-text
          suffixIcon="arrow-rightward"
          iconStyle="font-size: 18px"
          text="查看更多"
        ></u-text>
      </view>
    </view>

    <u-text type="info" margin="15px 0" text="超出隐藏"></u-text>
    <view class="u-demo-block__content">
      <u-text
        :lines="2"
        text="关于uView的取名来由，首字母u来自于uni-app首字母，uni-app是基于Vue.js，Vue和View(延伸为UI、视图之意)同音，同时view组件uni-app中 最基础，最重要的组件，故取名uView，表达源于uni-app和Vue之意，同时在此也对它们表示感谢。"
      ></u-text>
    </view>

    <u-text type="info" margin="15px 0" text="小程序开放能力"></u-text>
    <view class="u-demo-block__content">
      <u-text
        text="分享到微信"
        openType="share"
        type="success"
        @click="clickHandler"
      ></u-text>
    </view>
  </view>
</template>

<script setup lang="ts">
  import { Toast } from '@/utils/uniapi/prompt';

  const test = () => {
    console.log('test');
  };
  const clickHandler = () => {
    // #ifndef MP-WEIXIN
    Toast('请在微信小程序内查看效果');
    // #endif
  };
</script>

<style lang="scss">
  .u-page__text-item {
    margin-right: 10px;
    flex: 1;
  }

  .u-demo-block__content {
    @include flex;
    flex-direction: row;
    flex-wrap: wrap;
    align-items: center;
  }
</style>
